...
現状、`ng_dispatch.py` が Monitor の実行を制御しながら、NG 銘柄が出た際に Analyzer/Planning を subprocess で起動する役割も担っている。
...
- パイプライン制御は専用の `main_pipeline.py` が担い、各ブロックは自分の処理だけに集中する
...
flowchart TD
    ...
    subgraph PIPELINE["main_pipeline.py"]
        PO["エントリーポイント\nmain_pipeline.py"]
        ...
        subgraph PHASE2["Phase 2：Analyzer ブロック"]
            WAIT1["Phase 1 完了を待機"]
...
| 観点 | 旧実装 | 現行設計 |
|------|--------|---------|
| オーケストレーター | `Monitor/src/ng_dispatch.py` | `main_pipeline.py` |
| ブロック間の情報渡し | CLI 引数（ticker / span / mode）＋ DB | **DB のみ（伝言板方式）** |
| ティッカー処理 | 逐次（`for` ループ） | ブロック内並列（`asyncio.gather()`） |
| Monitor の責務 | 監視 ＋ Analyzer/Planning 起動 | **監視のみ** |
| NG 検出後の動作 | Monitor が即座に subprocess 起動 | NG 銘柄すべてを並列で Analyzer させる |
...
async def main():
    ...
    await run_analyzer_block(ng_archivelogs)      # Phase 3：NG 銘柄を並列議論
...
**Discord 通知（制御系）：** START / ERROR / COMPLETE 通知は `main_pipeline.py` が担う。
業務判定系の通知（【確認】【緊急】【警告】【朗報】）は Watch ブロックが担当する。

### Technical ブロック

`main.py` が watchlist 全銘柄のテクニカル指標を取得し、archive レコードを作成する。
TechnicalIndicatorFetcher（自作ライブラリ）を使用し、yfinance 経由で OHLCV データを取得して指標を算出する。

### Monitor ブロック

`monitor_batch.py` が watchlist 全銘柄を並列に `Monitor/src/main.py --ticker <T>` で実行する。
...
**配置：** `Watch/` ディレクトリとして新設（Monitor / Analyzer / Planning と同列のブロック）
...
| Analyzer | `archive`: `lanes`, `final_judge` | `active = True AND final_judge IS NOT NULL AND newplan_full IS NULL` |
...
例：Monitor 後に手動で DB レコードを確認し、Analyzer だけ単体で走らせることも可能になる。
...
全銘柄の Monitor 完了を待ってから Analyzer を一括開始するため、
watchlist が大きいほど最初の Analyzer 開始が遅れる。
