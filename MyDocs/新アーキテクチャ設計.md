...
| Discord 通知（業務判定系） | Planning が送信 | **Watch ブロックが送信（【確認】【緊急】【警告】【朗報】）** |
...
- **削除：** Discord 通知（URGENT / GOOD_NEWS / WARNING / CHECK）→ 通知責務は Watch ブロックへ移管済み
...
|---------|--------------------------|--------------------------------------|
| Monitor | `archive`: `id`, `created_at`, `ticker`, `mode`, `span`, `status`, `monitor`, `MotivationID`, `motivation_full`, `active`<br>`watchlist`: `MotivationID` | `active = True AND final_judge IS NULL` |
...
| Watch | `watchlist`: `motivation_summary`, `discussion_result`, `discussion_summary`, `newplan_summary`, `risk_flags`, `plan_comparison`, `stock_price`<br>`archive`: `active`（False に戻す） | —（パイプライン完了） |
