...
- **前方（早い方）: 3分** — cron が予定より早く発火することは稀なので狭くてよい
- **後方（遅い方）: 90分** — GitHub Actions の遅延は最大78分程度のため広くとる
...
時刻マッチングの許容窓（-3〜+90分）内で5分ポーリングが複数回走るため、
...
マッチ判定時に「同じラベルの最終実行が150分以内なら**スキップ**」する。
（許容窓が最大93分間あるため、それより長い150分に設定している）
...
15:07  cron発火 → diff = +7  → マッチ → last_runが10分前 → スキップ
  ...
16:30  cron発火 → diff = +90 → マッチ → last_runが93分前 → スキップ
16:35  cron発火 → diff = +95 → マッチしない（>90）→ 終了
```
