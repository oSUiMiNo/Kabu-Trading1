...
Monitor はこのデータを読んでからチェックを行い、Analyzer・Planning も定量データを参照できるようになる。
...
```
Monitor → Analyzer → Planning → Watch
          （NG のみ）
```

- Monitor が NG 判定時のみ archive レコードを作成
- Analyzer は自分用の別の archive レコードを作成
- `propagate_active_after_discussion()` で Monitor の archive から Analyzer の archive にデータをコピーしていた
...
archive ID: 20260319100000_AAPL（1レコード）
  ...
  Analyzer → lanes, final_judge を書き込み
...
| モード | コマンド例 | 用途 |
|--------|-----------|------|
| 全銘柄一括 | `python main.py` | パイプライン Phase 1 |
| 単一銘柄 | `python main.py --ticker AAPL` | ガード関数からの呼び出し |
...
Analyzer を単体実行する場合など、Technical フェーズを経由していない状況に対応する。
...
| 項目 | 現在 | 変更後 |
|------|------|--------|
| archive レコード | Analyzer が自分用の新レコードを作成 | Monitor が書き込んだ既存レコードに書き足す |
| propagate | Analyzer 後に monitor 等をコピー | 不要（同一レコードなので最初からデータがある） |
...
| Phase 2 | Analyzer | Monitor |
| Phase 3 | Planning | Analyzer |
| Phase 4 | Watch | Planning |
| Phase 5 | — | Watch |
| propagate_active_after_discussion | 実行する | 削除 |

### Analyzer 以降の開始条件（変更なし）
...
OK の archive は `active=False` のため下流に進まない。NG のみ Analyzer 以降へ流れる。

### 価格データの参照ルール

Technical が yfinance から取得した `archive.technical.latest_price` をパイプライン全体の正式な現在株価とする。
各ブロックは以下の優先順で価格を取得する。

| 優先順 | ソース | 取得方法 |
|--------|--------|---------|
| 1（最優先） | `archive.technical.latest_price` | Technical が yfinance API から取得した直近終値 |
| 2 | `archive.monitor.current_price` | Monitor が WebSearch で取得した価格 |
| 3（最終手段） | price-fetcher サブエージェント | Planning のみ。WebSearch で独自取得 |

**各ブロックの参照先**

| ブロック | 用途 | フォールバック |
|---------|------|---------------|
| Monitor | `monitor.current_price` と `price_change_pct` の算出に使用 | WebSearch で取得した価格 |
| Analyzer | `initial_prompt` に現在株価として注入 | `archive.monitor.current_price` |
| Planning | `data_checks.current_price` として使用 | `archive.monitor.current_price` → price-fetcher |
...
テクニカルデータがなくても Monitor・Analyzer は従来通り動作可能（テクニカルデータは補助材料）。
...
| 3 | `main.py` 実装 | 全銘柄一括 / 単一銘柄の2モード |
| 4 | `shared/supabase_client.py` に `ensure_technical_data()` 追加 | ガード関数 |
| 5 | `main_pipeline.py` に Phase 1（Technical）を追加 | Monitor の前に実行 |
| 6 | Monitor の仕様変更 | archive 作成を削除、Technical が作った既存レコードに書き込む形に変更 |
| 7 | Analyzer の仕様変更 | 既存 archive があればそこに書き足す。なければ従来通り作成 |
| 8 | `propagate_active_after_discussion()` の呼び出しを削除 | main_pipeline から除去 |
| 9 | Analyzer 側に `ensure_technical_data()` 呼び出しを追加 | 議論開始前にガード |
...
- 下流ブロック（Monitor / Analyzer / Planning）のプロンプトにテクニカルデータを組み込む具体的な実装
