...
```bash
python src/pipeline_orchestrator.py                    # 全パイプライン（Monitor → Discussion → Planning → Watch）
python src/pipeline_orchestrator.py --ticker NVDA      # 特定銘柄のみ
python src/pipeline_orchestrator.py --monitor-only     # 監視のみ（Discussion/Planning/Watch 起動しない）
python src/pipeline_orchestrator.py --market US        # 米国株のみ
...
| ファイル | 役割 |
|----------|------|
| src/pipeline_orchestrator.py | パイプラインオーケストレーター（Monitor → Discussion → Planning → Watch） |
...
両方を検出し、マッチしたら `pipeline_orchestrator.py` を起動する。
