...
## 採用寄り意見の選び方
`preferred_opinion` は `A` / `B` / `DRAW` / `UNKNOWN` のいずれか。

### 基本ルール
- `INCOMPLETE` の場合は `UNKNOWN`
- `AGREED` の場合
  - 両者とも概ね同じ判断なので `DRAW` を基本とする
  - ただし、一方の `data_limits` が明らかに重い場合は、より制約が少ない方を `A` または `B` にしてよい
- `DISAGREED` の場合
  - まず `data_limits` が軽い方を優先
  - 次に `confidence` が高い方を優先
  - それでも決まらなければ `DRAW`

### 注意
- `confidence` の数値だけで決めてはならない
- `major_risks` が重い側を無条件で採用してはならない
- `data_limits` が重い側の高確信は、そのまま信用しない
...
- `AGREED` の場合:
  - 共通の `supported_side`
- `DISAGREED` の場合:
  - `preferred_opinion` が `A` または `B` なら、その `supported_side`
  - `DRAW` なら `HOLD`
- `INCOMPLETE` の場合:
  - `null`
...
- `AGREED` の場合:
  ...
  - 片方だけあるならその値
- `DISAGREED` の場合:
  - `preferred_opinion` が `A` または `B` なら、その `confidence`
  - ただし **60 を上限の目安**として過信しない
  - `DRAW` なら `50`
- `INCOMPLETE` の場合:
  - `null`
...
decision:
  ...
  preferred_opinion: A | B | DRAW | UNKNOWN
